...
public class Seres extends Thread {
	...
	public Seres(String nombre) {
		super(nombre);
		esperanzaDeVida = (int) ((Math.random() * Constantes.VIDA_MAX) + 3);
		miFila = ColaSagrada.getInstance();
		System.out.println("nazco "+nombre+" con esperanza de "+esperanzaDeVida);
	}
	...
	public void run() {
		while (tamano < esperanzaDeVida) {
			if (turno) {
				System.out.println(getName()+" pido entrar");
